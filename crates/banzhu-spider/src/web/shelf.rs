...
) -> AppResult<Json<ApiResponse<Vec<crate::db::BookshelfRecord>>>> {
    let db = state.db.lock().await;
    let rows = db.get_bookshelf(q.group.as_deref())?;
...
pub(crate) async fn add_to_bookshelf(
    ...
    let db = state.db.lock().await;
    db.add_to_bookshelf(body.book_id, &group)?;
...
pub(crate) async fn update_shelf_group(
    ...
    let db = state.db.lock().await;
    db.update_bookshelf_group(book_id, &body.group)?;
...
pub(crate) async fn remove_from_bookshelf(
    ...
    let db = state.db.lock().await;
    db.remove_from_bookshelf(book_id)?;
...
pub(crate) async fn get_progress(
    State(state): State<Arc<AppState>>,
    Path(book_id): Path<i64>,
) -> AppResult<Json<ApiResponse<Option<crate::db::ReadingProgressRecord>>>> {
    let db = state.db.lock().await;
...
pub(crate) async fn update_progress(
    ...
    let db = state.db.lock().await;
    db.update_progress(book_id, body.chapter_order, body.page_index)?;
    Ok(ok_response(json!({
        "book_id": book_id,
